...
public static class Rfc1123StringExtensions
{
    ...
    private static readonly HashSet<char> AllowedChars = [.. "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-."];
    ...
    public static string ToRFC1123(this string input)
    {
        ...
        foreach (char c in input)
        {
            if (c == EscapeChar)
            {
                _ = sb.Append(EscapeChar).Append(EscapeChar); // Escape the escape char
            }
            ...
            else
            {
                ...
                foreach (byte b in utf8Bytes)
                {
                    ...
                    _ = sb.Append(EscapeChar).Append(b.ToString("X2"));
                }
            }
        }
    ...
    private static int Escape(string input, StringBuilder resultBuilder, MemoryStream byteStream, int i)
    {
        ...
        if (i + 1 >= input.Length)
        {
            ...
            throw new FormatException($"Invalid escape sequence: trailing '{EscapeChar}' at end of string.");
        }

        char nextChar = input[i + 1];

        if (nextChar == EscapeChar) // It's '__', representing a literal '_'
        {
            // Decode any pending bytes before appending the literal char
            DecodePendingBytes(byteStream, resultBuilder);
            _ = resultBuilder.Append(EscapeChar);
            i += 2; // Consumed '__'
        }
        else // Should be '_XX' (hex byte representation)
        {
            if (i + 2 >= input.Length)
            {
                ...
                throw new FormatException($"Invalid escape sequence: '{EscapeChar}{nextChar}' is incomplete at end of string.");
            }
